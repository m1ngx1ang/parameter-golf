...
RUN_NAME="${RUN_NAME:-sp8192-recurmlplora-8xh100-$(date -u +%Y%m%d-%H%M%S)}"
REPO_REF="${REPO_REF:-$(git symbolic-ref --quiet --short HEAD || git rev-parse HEAD)}"
RUN_CONFIG_PATH="${RUN_CONFIG_PATH:-${RECORD_DIR}/.runpod/runpod-8xh100-sp8192.env}"
...
setup_command='python3 -m pip install --upgrade pip && python3 -m pip install -r requirements.txt && python3 -m pip install -r records/track_10min_16mb/2026-04-12_SP8192_RecurMLPLoRA/runpod_requirements.txt && if ! python3 -c "from flash_attn_interface import flash_attn_func" >/dev/null 2>&1; then python3 -m pip install flash_attn_3 --no-deps --find-links https://windreamer.github.io/flash-attention3-wheels/cu128_torch291/ || (cd /tmp && rm -rf flash-attention && git clone --depth 1 https://github.com/Dao-AILab/flash-attention.git && cd flash-attention/hopper && NVCC_THREADS=2 python3 setup.py install); fi && cd /workspace/parameter-golf && python3 -c "from flash_attn_interface import flash_attn_func; import brotli; print(\"deps ok\")" && rm -f data/manifest.json && MATCHED_FINEWEB_REPO_ID=kevclark/parameter-golf python3 data/cached_challenge_fineweb.py --variant sp8192'
job_command="cd ${RECORD_DIR} && DATA_DIR=/workspace/parameter-golf/data RUN_ID=${RUN_NAME} SEED=${SEED:-42} MAX_WALLCLOCK_SECONDS=${MAX_WALLCLOCK_SECONDS:-600} VOCAB_SIZE=8192 MODEL_DIM=512 NUM_LAYERS=11 NUM_HEADS=8 NUM_KV_HEADS=4 MLP_MULT=4.0 QK_GAIN_INIT=5.25 NUM_LOOPS=2 LOOP_START=3 LOOP_END=5 ENABLE_LOOPING_AT=${ENABLE_LOOPING_AT:-0.35} PARALLEL_RESIDUAL_START=7 WARMDOWN_FRAC=0.72 WARMUP_STEPS=20 TRAIN_BATCH_TOKENS=786432 TRAIN_SEQ_LEN=2048 VAL_BATCH_TOKENS=524288 EVAL_SEQ_LEN=2048 VAL_LOSS_EVERY=4000 TRAIN_LOG_EVERY=500 SLIDING_WINDOW_ENABLED=1 EVAL_STRIDE=64 MATRIX_LR=0.022 SCALAR_LR=0.02 MUON_MOMENTUM=0.99 MUON_BACKEND_STEPS=5 MUON_MOMENTUM_WARMUP_START=0.92 MUON_MOMENTUM_WARMUP_STEPS=1500 MUON_ROW_NORMALIZE=1 ADAM_WD=0.02 MUON_WD=0.095 EMBED_WD=0.085 EMA_DECAY=0.9965 MATRIX_BITS=6 EMBED_BITS=8 MATRIX_CLIP_SIGMAS=12.85 EMBED_CLIP_SIGMAS=20 COMPRESSOR=brotli GPTQ_CALIBRATION_BATCHES=64 GPTQ_RESERVE_SECONDS=12 TTT_ENABLED=1 TTT_LR=0.005 TTT_EPOCHS=3 TTT_MOMENTUM=0.9 TTT_CHUNK_TOKENS=32768 MLP_LORA_RANK=${MLP_LORA_RANK:-4} MLP_LORA_ALPHA=${MLP_LORA_ALPHA:-4.0} MLP_LORA_LR=${MLP_LORA_LR:-0.04} MLP_LORA_WD=${MLP_LORA_WD:-0.0} MLP_LORA_LAYERS=${MLP_LORA_LAYERS:-3,4,5} ENABLE_MLP_LORA_AT=${ENABLE_MLP_LORA_AT:-${ENABLE_LOOPING_AT:-0.35}} STABLE_RESID_MIX=${STABLE_RESID_MIX:-1} STABLE_RESID_CARRY_INIT=${STABLE_RESID_CARRY_INIT:-0.99} torchrun --standalone --nproc_per_node=8 train_gpt.py"
...
RUNPOD_CUSTOM_SSH_BOOTSTRAP=auto
