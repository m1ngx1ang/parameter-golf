...
class Hyperparameters:
    ...
    recurrent_layers = int(os.environ.get("RECURRENT_LAYERS", 3))
    postlude_layers = int(os.environ.get("POSTLUDE_LAYERS", 2))
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        total_layers = h.prelude_layers + h.recurrent_layers + h.postlude_layers
        layer_mlp_mults = (
            [h.prelude_mlp_mult] * h.prelude_layers
            + [h.recurrent_mlp_mult] * h.recurrent_layers
            + [h.postlude_mlp_mult] * h.postlude_layers
        )
        ...
        if h.parallel_residual_start >= 0:
            for i in range(h.parallel_residual_start, total_layers):
                self.blocks[i].parallel = True
        self.prelude_indices = list(range(h.prelude_layers))
        recurrent_start = h.prelude_layers
        recurrent_end = recurrent_start + h.recurrent_layers
        self.recurrent_indices = list(range(recurrent_start, recurrent_end))
        self.postlude_indices = list(range(recurrent_end, total_layers))
        self.num_skip_weights = min(len(self.prelude_indices), len(self.postlude_indices))
        ...
        self.recur_carry_log = nn.Parameter(
            torch.full((h.model_dim,), raw_carry_init, dtype=torch.float32)
        )
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for p in os.environ.get(
        ...
        "attn_scale,mlp_scale,resid_mix,q_gain,recur_carry_log,recur_input_scale,skip_weight,skip_gates",
    ).split(",")
    if p
)
...
class Optimizers:
    def __init__(self, h, base_model):
        ...
        scalar_params.extend([base_model.recur_carry_log, base_model.recur_input_scale])
...
def main():
    ...
    if h.prelude_mlp_mult <= 0.0:
        raise ValueError(
            f"PRELUDE_MLP_MULT must be positive, got {h.prelude_mlp_mult}"
        )
    if h.recurrent_mlp_mult <= 0.0:
        raise ValueError(
            f"RECURRENT_MLP_MULT must be positive, got {h.recurrent_mlp_mult}"
        )
    if h.postlude_mlp_mult <= 0.0:
        raise ValueError(
            f"POSTLUDE_MLP_MULT must be positive, got {h.postlude_mlp_mult}"
        )
    if int(h.prelude_mlp_mult * h.model_dim) < 1:
        raise ValueError(
            f"PRELUDE_MLP_MULT*MODEL_DIM must be >= 1, got {h.prelude_mlp_mult * h.model_dim}"
        )
    if int(h.recurrent_mlp_mult * h.model_dim) < 1:
        raise ValueError(
            f"RECURRENT_MLP_MULT*MODEL_DIM must be >= 1, got {h.recurrent_mlp_mult * h.model_dim}"
        )
    if int(h.postlude_mlp_mult * h.model_dim) < 1:
        raise ValueError(
