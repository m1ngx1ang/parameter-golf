...
RUN_NAME="${RUN_NAME:-sp8192-pre2-rec3-post2-looplora-${RUNPOD_GPU_COUNT}xh100-$(date -u +%Y%m%d-%H%M%S)}"
...
# LoRA knobs for loop specialization in recurrent stage.
lora_post_loop_env="MLP_LORA_RANK=${MLP_LORA_RANK:-8} MLP_LORA_ALPHA=${MLP_LORA_ALPHA:-8.0} LOOP_SETTLE_FRAC=${LOOP_SETTLE_FRAC:-0.10} MLP_LORA_WARMUP_STEPS=${MLP_LORA_WARMUP_STEPS:-400} MLP_LORA_LR=${MLP_LORA_LR:-0.02} MLP_LORA_WD=${MLP_LORA_WD:-0.0}"

job_command="cd ${RECORD_DIR} && DATA_DIR=/workspace/parameter-golf/data RUN_ID=${RUN_NAME} SEED=${SEED:-42} ITERATIONS=${ITERATIONS} WARMDOWN_FRAC=${WARMDOWN_FRAC} WARMUP_STEPS=${WARMUP_STEPS} TRAIN_BATCH_TOKENS=${TRAIN_BATCH_TOKENS} TRAIN_SEQ_LEN=${TRAIN_SEQ_LEN} TRAIN_LOG_EVERY=${TRAIN_LOG_EVERY} MAX_WALLCLOCK_SECONDS=${MAX_WALLCLOCK_SECONDS} VAL_BATCH_TOKENS=${VAL_BATCH_TOKENS} EVAL_SEQ_LEN=${EVAL_SEQ_LEN} VAL_MAX_TOKENS=${VAL_MAX_TOKENS:-0} VAL_LOSS_EVERY=${VAL_LOSS_EVERY} SLIDING_WINDOW_ENABLED=${SLIDING_WINDOW_ENABLED} VOCAB_SIZE=${VOCAB_SIZE} PRELUDE_LAYERS=${PRELUDE_LAYERS} RECURRENT_LAYERS=${RECURRENT_LAYERS} POSTLUDE_LAYERS=${POSTLUDE_LAYERS} NUM_RECUR_LOOPS=${NUM_RECUR_LOOPS} MODEL_DIM=${MODEL_DIM} NUM_KV_HEADS=${NUM_KV_HEADS} NUM_HEADS=${NUM_HEADS} PRELUDE_MLP_MULT=${PRELUDE_MLP_MULT} RECURRENT_MLP_MULT=${RECURRENT_MLP_MULT} POSTLUDE_MLP_MULT=${POSTLUDE_MLP_MULT} TIE_EMBEDDINGS=${TIE_EMBEDDINGS} LOGIT_SOFTCAP=${LOGIT_SOFTCAP} ROPE_BASE=${ROPE_BASE} ROPE_DIMS=${ROPE_DIMS} LN_SCALE=${LN_SCALE} QK_GAIN_INIT=${QK_GAIN_INIT} ENABLE_LOOPING_AT=${ENABLE_LOOPING_AT} RECUR_CARRY_INIT=${RECUR_CARRY_INIT} RECUR_INPUT_SCALE_INIT=${RECUR_INPUT_SCALE_INIT} PARALLEL_RESIDUAL_START=${PARALLEL_RESIDUAL_START} MIN_LR=${MIN_LR} EMBED_LR=${EMBED_LR} HEAD_LR=${HEAD_LR} TIED_EMBED_LR=${TIED_EMBED_LR} TIED_EMBED_INIT_STD=${TIED_EMBED_INIT_STD} MATRIX_LR=${MATRIX_LR} SCALAR_LR=${SCALAR_LR} MUON_MOMENTUM=${MUON_MOMENTUM} MUON_BACKEND_STEPS=${MUON_BACKEND_STEPS} MUON_MOMENTUM_WARMUP_START=${MUON_MOMENTUM_WARMUP_START} MUON_MOMENTUM_WARMUP_STEPS=${MUON_MOMENTUM_WARMUP_STEPS} MUON_ROW_NORMALIZE=${MUON_ROW_NORMALIZE} BETA1=${BETA1} BETA2=${BETA2} ADAM_EPS=${ADAM_EPS} GRAD_CLIP_NORM=${GRAD_CLIP_NORM} EVAL_STRIDE=${EVAL_STRIDE} ADAM_WD=${ADAM_WD} MUON_WD=${MUON_WD} EMBED_WD=${EMBED_WD} EMA_DECAY=${EMA_DECAY} TTT_ENABLED=${TTT_ENABLED} TTT_LR=${TTT_LR} TTT_EPOCHS=${TTT_EPOCHS} TTT_MOMENTUM=${TTT_MOMENTUM} TTT_CHUNK_TOKENS=${TTT_CHUNK_TOKENS} COMPRESSOR=${COMPRESSOR} GPTQ_CALIBRATION_BATCHES=${GPTQ_CALIBRATION_BATCHES} GPTQ_RESERVE_SECONDS=${GPTQ_RESERVE_SECONDS} MATRIX_BITS=${MATRIX_BITS} EMBED_BITS=${EMBED_BITS} MATRIX_CLIP_SIGMAS=${MATRIX_CLIP_SIGMAS} EMBED_CLIP_SIGMAS=${EMBED_CLIP_SIGMAS} ${lora_post_loop_env} torchrun --standalone --nproc_per_node=${TORCH_NPROC_PER_NODE} train_gpt.py"
