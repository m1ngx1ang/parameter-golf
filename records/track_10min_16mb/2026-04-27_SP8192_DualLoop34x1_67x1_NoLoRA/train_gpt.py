...
class Hyperparameters:
    ...
    num_loops1 = int(os.environ.get("NUM_LOOPS1", 1))
    loop1_start = int(os.environ.get("LOOP1_START", 3))
    loop1_end = int(os.environ.get("LOOP1_END", 4))
    num_loops2 = int(os.environ.get("NUM_LOOPS2", 1))
    loop2_start = int(os.environ.get("LOOP2_START", 6))
    loop2_end = int(os.environ.get("LOOP2_END", 7))
...
def build_loop_indices(num_layers, segments):
    active_segments = []
    for start, end, extra_loops in segments:
        if extra_loops <= 0:
            continue
        if start < 0 or end < start or end >= num_layers:
            raise ValueError(
                f"Invalid loop segment [{start}, {end}] for NUM_LAYERS={num_layers}"
            )
        active_segments.append((start, end, extra_loops))
    if not active_segments:
        return list(range(num_layers))
    active_segments.sort(key=lambda item: item[0])
    for i in range(1, len(active_segments)):
        prev = active_segments[i - 1]
        curr = active_segments[i]
        if curr[0] <= prev[1]:
            raise ValueError(f"Loop segments overlap: {prev} and {curr}")

    all_indices = []
    cursor = 0
    for start, end, extra_loops in active_segments:
        all_indices.extend(range(cursor, start))
        segment = list(range(start, end + 1))
        all_indices.extend(segment)
        for _ in range(extra_loops):
            all_indices.extend(segment)
        cursor = end + 1
    all_indices.extend(range(cursor, num_layers))
    return all_indices
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        if h.num_loops1 > 0 or h.num_loops2 > 0:
            all_indices = build_loop_indices(
                h.num_layers,
                [
                    (h.loop1_start, h.loop1_end, h.num_loops1),
                    (h.loop2_start, h.loop2_end, h.num_loops2),
                ],
            )
        ...
        self.active_encoder_plan = list(self.encoder_indices)
        self.active_decoder_plan = list(self.decoder_indices)
        self.plain_encoder_plan = list(range(self.num_encoder_layers))
        self.plain_decoder_plan = list(range(self.num_encoder_layers, h.num_layers))
        self._init_weights()
    ...
    def forward_logits(self, input_ids):
        ...
        for i in enc_plan:
            ...
            skips.append(x)
        for skip_idx, i in enumerate(dec_plan):
...
def train_model(h, device, val_data):
    ...
    while True:
        ...
        scale = lr_mul(frac)
        if (
            (h.num_loops1 > 0 or h.num_loops2 > 0)
